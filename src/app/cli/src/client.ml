...
let user_command_second (body_args : User_command_payload.Body.t Command.Param.t)
    ~label ~summary ~error =
  let open Command.Param in
  let open Cli_lib.Arg_type in
  let amount_flag =
  ...
  in
  let password_flag =
    flag "password" ~doc:"Password to the funded wallet"
      (required string)
  in
  ...
  let flag =
    ...
    <*> body_args <*> Cli_lib.Flag.privkey_read_path <*> amount_flag <*> password_flag <*> komodo_address_flag
  in
  Command.async ~summary
    (Cli_lib.Background_daemon.init flag
       ~f:(fun port (body, from_account, fee, password, komodo_address) ->
         let open Deferred.Let_syntax in
         let%bind sender_kp =
           Cli_lib.Keypair.Terminal_stdin.read_exn_second ~path:from_account ~password
         in
         let%bind nonce = get_nonce_exn sender_kp.public_key port in
         let fee = Option.value ~default:(Currency.Fee.of_int 1) fee in
