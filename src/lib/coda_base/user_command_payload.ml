...
(* komodo *)
(* let create_with_komodo ~fee ~nonce ~memo ~body ~komodo_address : t = {common= {fee; nonce; memo}; body; komodo_address} *)
...
(* komodo *)
let memo_to_string memo = Binable.to_string (module Memo) memo |> B64.encode
let memo_of_string s = B64.decode s |> Binable.of_string (module Memo)
(* komodo *)